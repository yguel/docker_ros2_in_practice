...
alias ros2_fbuild='ros2_dep && colcon build --executor sequential --cmake-args -DCMAKE_BUILD_TYPE=Release --symlink-install && source install/setup.bash'
alias ros2_fbuild_debug='ros2_dep && colcon build --executor sequential --cmake-args -DCMAKE_BUILD_TYPE=Debug --symlink-install && source install/setup.bash'
alias ros2_fbuild_reldebug='ros2_dep && colcon build --executor sequential --cmake-args -DCMAKE_BUILD_TYPE=RelWithDebInfo --symlink-install && source install/setup.bash'

function ros2_fbuild_only {
ros2_dep
colcon build --executor sequential --cmake-args -DCMAKE_BUILD_TYPE=Release --symlink-install --packages-select $@
source install/setup.bash
}

function ros2_fbuild_only_debug {
ros2_dep
colcon build --executor sequential --cmake-args -DCMAKE_BUILD_TYPE=Debug --symlink-install --packages-select $@
source install/setup.bash
}

function ros2_fbuild_except {
old="$IFS"
IFS='|'
all_except_those="$*"
IFS=$old
...
#echo $reg
ros2_dep
colcon build --executor sequential --cmake-args -DCMAKE_BUILD_TYPE=Release --symlink-install --packages-select-regex $reg
source install/setup.bash
}
...
# To be sure that local ROS2 setup is loaded
function myros2 {
export COLCON_EXTENSION_BLOCKLIST=colcon_core.event_handler.desktop_notification
